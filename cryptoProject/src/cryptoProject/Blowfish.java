...
public class Blowfish {
	//s boxes values in hex
	 String sBoxes[][] 
		    ...
		    // Sub keys initialization with digits of pi. 
		        ...
		        for (int i = 0; i < 8; i += 2) { 
		            ...
		            long col 
		                = Long.parseUnsignedLong( 
		                    hexToBin( plainText.substring(i, i + 2)), 2); 
		            a[i / 2] = sBoxes[i / 2][(int)col]; 
		        } 
		        ans = addBin(a[0], a[1]); 
		        ans = xor(ans, a[2]); 
		        ans = addBin(ans, a[3]); 
		    ...
		    { 
		     ...
		     Blowfish(String key,MyElGamal elgamal) throws FileNotFoundException, IOException 
		    { 
		      ...
		      //  String plainText = "123456abcd132536"; 
		        ...
		        //new input file 
		        File inputFile= new File("C:\\Users\\User\\git\\cryptoProject\\cryptoProject\\src\\cryptoProject\\input.txt");
		       
		        List <Character> plainText=new ArrayList<>();       //plain text list
		        List <String> encryptedText=new ArrayList<>();      //encrypted text list
		        List <String> decryptedTextList=new ArrayList<>();  //decrypted text list
		        Reader reader = new FileReader(inputFile);          //new file reader
		        char [] chars = new char[8];                        //used to save a block of text
		        try {
		        	//read from file blocks of info to encrypt and decrypt
		        	System.out.println("");
		        	System.out.println("BlowFish");
					for(int len;(len=reader.read(chars))>0;)
					{
						/*convert text from ascii to hex*/
						...
						for(char c : chars)
						{
							plainText.add(c);//save plain text in list
							int i = (int)c; 
							String s1=Integer.toHexString(i);
							String s2="0";
							if(s1.length()!=2)//in case of enter 
							{
								s2+=s1;
								s1=s2;
							}
						...
						/********************/
						String cipherText = encrypt(builder.toString()); //encrypt block
						encryptedText.add(cipherText);//save cipher text in list
						
						//checking if Alice is the sender and that the signature is verified if yes Bob decrypt the cipher text 
						if(elgamal.getVerifiedMsg().contentEquals("Your Signature is verified ;) , you are Alice!"))
						{
					    String decryptedText = decrypt(cipherText);//decrypt
					    /*convert decrypted text from hex to ascii*/
					    StringBuilder builder2 = new StringBuilder();
					    for(int i=0;i<decryptedText.length();i=i+2)
					    {
					    	String s = decryptedText.substring(i,i+2);
					    	int n= Integer.valueOf(s,16);
					    	builder2.append((char)n);
					    }
					    /***********************/
				    	  decryptedTextList.add(builder2.toString());//save decryptrd text
				    	 
				
					
						}
						else { 
							System.out.println(elgamal.getVerifiedMsg());
						}
					}
					 reader.close();  //close file reader
					 /*print plain text, cipher text and decrypted text*/
					 ...
					 System.out.println("Reading plain text from file");
						...
						System.out.println("");
						System.out.println("Sending cipher text to Bob");
						System.out.println("Sending signature to Bob");////
						System.out.println("Alice's signature is ( "+ elgamal.getR() + " , " + elgamal.getS2() + " )");//
						System.out.println("*************************************");
						System.out.println("*****************Bob*****************");
						System.out.println("Bob received the cipher text");
						System.out.println("Bob received the signature");
						System.out.println(elgamal.getVerifiedMsg());//
						System.out.println("");
						...
						{
							System.out.print(decryptedTextList.get(i));
						}
